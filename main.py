...
class Application(object):
    ...
    def get_headers(self, env):
        ...
        for k, v in env.iteritems():
            ...
            self.Request.raw += "%s: %s\n" % (k, v)  # capture request for logging
            if k == 'REQUEST_SCHEME':
    ...
    def get_request_body(self, env):
        ...
        self.Request.raw += data  # capture request for logging
    ...
    def set_headers(self):
        ...
        for k, v in self.Response.headers.iteritems():
            ...
            if k.lower() == 'access-control-allow-origin': access_control_allow_origin = True
            h.append((k, str(v)))
    ...
    def __init__(self, env):
        try:
            ...
            try:
                ...
                self.Request.raw = ''  # capture request for logging
                self.Request.realaddr = self.real_addr(env)
                self.Response.raw = ''  # capture response for logging
                self.Response.status_code = 200
                self.Response.headers = {'Content-Type': 'text/plain'}
                self.Response.content = ''
                try:
                    handler, params = self.get_handler(env)
                    self.Request.urlparams = params
